...
contract Distribute is Ownable {
    ...
    bool _inited = false;
    function addInitParames(uint _tokenId, uint _canClaimTotal) public onlyOwner {
        require(_inited == false, "can't add parames");
        require(_tokenId != uint(0), "tokenId error");
        require(_canClaimTotal > 0, "canClaimTotal error");
    ...
    function claim() hasClaimRight() public {
        ...
        uint _days = SafeMath.div((block.timestamp - gameStartTime), SECONDS_PER_DAY) + 60;
        uint _canClaimeNumTotal = SafeMath.div(_days, 30);
        ...
        nextClaimTime = gameStartTime + (_canClaimeNumTotal - 1) * (30 days);
        if(_needSendTotal > 0){
